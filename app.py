...
def generate_all_reports(df_assign: pd.DataFrame, df_monitor: pd.DataFrame, taluk: str) -> Dict:
    ...
    try:
        ...
        # ── Detect enumerator column (must be before village + VAO blocks) ──
        enu = next((c for c in df_m.columns if c.strip().lower()=="enu name"), None)
        if enu is None and nc > AppConfig.MONITOR_COL_ENumerator: enu = df_m.columns[AppConfig.MONITOR_COL_ENumerator]
        if enu is None: raise ValueError("No enumerator name column found.")

        # ── Detect village columns ───────────────────────────────────
        # Assignment file: Column 2 (index 1, 1-based col 2)
        # Task Monitoring file: Column 4 (index 3, 1-based col 4)
        vil_a = df_a.columns[AppConfig.ASSIGN_COL_VILLAGE] if df_a.shape[1] > AppConfig.ASSIGN_COL_VILLAGE else None
        vil_m = df_m.columns[AppConfig.MONITOR_COL_VILLAGE] if df_m.shape[1] > AppConfig.MONITOR_COL_VILLAGE else None
        logger.info("Village cols → Assignment[%s], Monitoring[%s]", vil_a, vil_m)

        # ── Build village-wise assignment table ─────────────────────
        # Keep a copy for village report before deleting large frames
        if vil_a is not None and vil_m is not None:
            va_raw = df_a[["User", vil_a, t_col]].copy()
            va_raw["VAO_CK"]  = va_raw["User"].apply(clean_name)
            va_raw["Village"] = va_raw[vil_a].astype(str).str.strip()
            va_raw[t_col]     = pd.to_numeric(va_raw[t_col], errors="coerce").fillna(0)

            vm_raw = pd.DataFrame({
                "VAO_CK":  df_m[enu].astype(str).apply(clean_name),
                "Village": df_m[vil_m].astype(str).str.strip(),
                "GW_Done": gw_s,
            })
            vm_grp = vm_raw.groupby(["VAO_CK","Village"])["GW_Done"].sum().reset_index()
            del vm_raw
            have_village_data = True
        else:
            va_raw = vm_grp = None
            have_village_data = False
            logger.warning("Village column not found — village report skipped for %s", taluk)
        ...
        if have_village_data:
            try:
                # Merge assignment and monitoring at village level
                vil_fin = pd.merge(
                    va_raw.rename(columns={t_col:"GW_Assigned"}),
                    vm_grp,
                    on=["VAO_CK","Village"],
                    how="left"
                ).fillna(0)
                vil_fin["GW_Done"] = pd.to_numeric(vil_fin["GW_Done"], errors="coerce").fillna(0).astype(int)
                vil_fin["GW_Assigned"] = pd.to_numeric(vil_fin["GW_Assigned"], errors="coerce").fillna(0).astype(int)
